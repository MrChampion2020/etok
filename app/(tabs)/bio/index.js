...
const Index = () => {
  ...
  const fetchUserDescription = async () => {
    try {
      const response = await axios.get(
        `http://192.168.137.40:3000/users/${userId}`
      );
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.header}>Champion Aden</Text>
      <Carousel
        data={profileImages}
      ...
      />
      <Pagination
        dotsLength={profileImages.length}
      ...
      />
      <Button title="Update Description" onPress={updateUserDescription} />
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <Pressable
            ...
            onPress={() => handleToggleTurnOn(item.id)}
          >
